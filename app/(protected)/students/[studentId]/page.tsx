...
export default function EditStudentPage() {
  ...
  const [initialValues, setInitialValues] = useState<StudentFormState>(EMPTY_VALUES);
    ...
    async function fetchStudent() {
      try {
        ...
        if (!res.ok || json.errors) {
          throw new Error(
            json.errors?.[0]?.message || "Failed to fetch student"
          );
        }

        const student = json.data?.student?.data;
        if (!student) {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            {touched.admissionDate && errors.admissionDate && (
              <p className="mt-1 text-xs text-red-500">{errors.admissionDate}</p>
            )}
          </div>
        </div>
